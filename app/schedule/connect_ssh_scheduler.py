"""SSH test and execute playlist functions for Scheduled tasks"""
import os
...
from app.schedule.mailer_scheduler import send_email_scheduler
from app.database.createdb import db
from app.database.models import Runhistory
log = logging.getLogger('app')
#paramiko.util.log_to_file("app/logs/paramiko.log", level = "DEBUG")
...
def test_ssh(cmd):
    ...
    try:
        ...
        try:
            ...
            raise
#  For scheduled task we ommit this check, as we want to make sure that scheduled task executes even if somebody is already running the same playbook otherwise!
#        else:
#            try:
...
def run_ssh_scheduler(cmd, runlog_path, ldap_user, email):
    """ Function that executes ansible playbook for an Scheduled task call """
    ...
    try:
        record = Runhistory(user=ldap_user, type=_type, status=_status, logfile=_logfile)
    ...
    if len(email) != 0:
        try:
            ...
            send_email_scheduler(recipient_email, filename, ldap_user)
        except Exception as e:
            log.error('Mail to recipients {} failed because of {}'.format(recipient_email, (str(e))))
        else:
            log.info('Mail sent succesfully to user {} with Scheduled task runlog {}'.format(recipient_email, filename))
